...
class CategoriController extends Controller
{
    ...
    {
        $category = Categori::where('id', $request->id)->update([
            'name' => $request->name,
            'is_publish' => $request->is_publish,
        ]);
        if ($category) {
    ...
    {
        $delete = Categori::where('id',$request->id)->first();
        
        DeleteNotifJob::dispatch($delete);
        $delete->delete();
